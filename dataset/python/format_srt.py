...
def replace(sp_dict, word, log_file, log_info):
	...
	ikfix_word = word.replace('Ik', 'k')
	if sp_dict.check(ikfix_word) or pos_tag([ikfix_word]) == 'NNP':
		return ikfix_word
...
log_file = codecs.open(osp.join(logs_dir, 'spell_check_log.txt'), 'w', encoding='utf-8')
			...
			for i in reversed(xrange(num_subs)):
				...
				if '\n' in subs[i].text:
					...
					for j in xrange(len(lines)):
						new_sub = pysrt.SubRipItem(0, start=str(subs[i].start), end=str(dummy.start), text=lines[j])
